...
# python ./bug_duplicates_test_clusterID.py --env Prod --clusterID 3
...
def parse_options():
    ...
    parser.add_argument("--clusterID", default="", help ='Cluster ID', type=str, metavar='v')
...
def bugDuplicate_view_set(e, cluster_id, stops, max_seq_length, text_cols, embedding_dim, cut_off):
	...
	filename = '/data/csap_models/bugDups/usecase2/model3_lstm_cnn_cat_' + str(cluster_id) + '.h5'
	...
	res_coll_name = "BugDupsTestSet_" + str(cluster_id) + "_results"
	collection = db[res_coll_name]
	collection.drop()
	final_df = pd.DataFrame()
	f = e.copy()
	#e = e[e['DUPLICATE_OF'].notnull()]
	duplicate_bugs_length = e.shape[0]
	#identifiers = list(f['IDENTIFIER'])
	for j in range(0, duplicate_bugs_length):
		b1 = e.iloc[j, ]
		duplicate_of = b1['DUPLICATE_OF']
		identifier = b1['IDENTIFIER']
		if(duplicate_of != None):
			df = f[(f['PRODUCT'] == b1['PRODUCT']) & (f['PROJECT'] == b1['PROJECT'])]#e[(e['PRODUCT2'] == b1['PRODUCT']) & (e['PROJECT2'] == b1['PROJECT'])
			df1 = get_test_data(df, b1)
			print(j, duplicate_bugs_length, df1.shape[0], duplicate_of)
...
if __name__ == "__main__":
	...
	e = e[e['PROJECT'] == 'CSC.sys']
	df = e.copy()
	cut_off = 90
	final_df = bugDuplicate_view_set(e, cluster_id, stops, max_seq_length, text_cols, embedding_dim, cut_off)
...
key = "csap_prod_database"
db = get_db(settings, key)
collection = db['BugDupsTestSet_4_results']
cursor = collection.find({})
