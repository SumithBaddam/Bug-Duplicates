#For filtering the dataframe only for those project and product. No need of running the cat predictions model.
import pymongo
import pandas as pd
import random
import configparser
...
for j in range(0, duplicate_bugs_length):
	...
	df = df[(df['PROJECT'] == b1['PROJECT']) & (df['PRODUCT'] == b1['PRODUCT'])]
...
res_coll_name = 'BugDupsTestSet_' + str(cluster_id) + '_results_1'
...
collection = db['BugDupsTestSet_3_results']
cursor = collection.find({})
