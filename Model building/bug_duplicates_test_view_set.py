...
def bugDuplicate_view_set(e, cluster_id, stops, max_seq_length, text_cols, embedding_dim, view_id, query_id, cut_off):
	...
	for j in range(0, duplicate_bugs_length):
		...
		result['DUP_Headline'] = df1['DUP_Headline']
		...
		if(v != 0):
			...
			cos_similarities = []
			t1 = ['Headline']
			t2 = list(result['DUP_Headline'])
			vector1 = text_to_vector(t1)
			for i in range(len(t2)):
				vector2 = text_to_vector(t2[i])
				cos_similarities.append(get_cosine(vector1, vector2))
			...
			if(len(idx) > 0):
				...
				df2['DUPLICATE_LIST'] = ' '.join(list(result['DUPLICATE_OF']))
				df2['TEXT_PROBABILITIES'] = ' '.join(str(x) for x in list(result['pred_text']))
				df2['COSINE_SIMILARITY'] = ' '.join(str(x) for x in list(result['cos_sim']))
				#df2['COSINE_SIMILARITY'] = ' '.join(str(x) for x in values)
				#df2['CAT_PROBABILITIES'] = ' '.join(str(x) for x in list(result.iloc[0:v]['pred_cat']))
				df2['actual_duplicate'] = duplicate_of
				df2['STATUS'] = status
				df2['SUBMITTED_DATE'] = sub_date
		...
		if(final_df.shape[0] > 100):
			...
			####Remove the bugIds already present in results
			final_df = final_df[~final_df['IDENTIFIER'].isin(a)]
	...
	for q in queries:
		...
		if(e.shape[0] > 0):
			...
			if(e.shape[0] > 0):
				...
				print("No new bugs added to this view")
		else:
			print("No data for this queryID")