...
def get_test_data(df, test_df, w2vmodel, words, vocabulary, inverse_vocabulary, cluster_id, stops):
	...
	print(sentence)
	wrdEmbSet_int = build_test_data_text(sentence, w2vmodel, words, vocabulary, inverse_vocabulary, cluster_id, stops)
...
if __name__ == "__main__":
    ...
    project = "CSC.content-security" #Get from the GUI
    cluster_id = get_cluster_id(db, project) #Find the cluster based on the project.

    final_df = bugDuplicate_new_input(cluster_id) #b1 is the input from GUI
